...
export const BUILT_IN_TOOL_SETS: ReadonlyMap<string, readonly string[]> =
	new Map([
		...
		["file_write", ["write", "apply_patch", "mv", "rm"]],
		[
			"git_read",
			[
				"git_status",
...
export const BASE_FOCUS_DEFINITIONS: readonly FocusDefinition[] = [
	...
	},
	{
		...
		prompt:
			"You are in edit focus. Make focused file changes with read/write/apply_patch/mv/rm. Keep changes minimal.",
		tools: [],
	...
	},
	{
		name: "yolo",
		description:
			"Shell-enabled focus for manual use only. Can read, write, apply patches, and run shell commands.",
		prompt:
			"You are in yolo focus. Use read, write, apply_patch, and bash to inspect and modify files and run shell commands. Avoid exposing secrets or credentials. If a command may print secrets, use a safer command or mask sensitive output. Ask for confirmation for gray-area destructive or privacy-sensitive actions.",
		tools: ["read", "write", "apply_patch", "bash"],
