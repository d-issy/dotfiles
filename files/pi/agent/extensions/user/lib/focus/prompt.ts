...
function buildFocusSystemPrompt(focus: FocusController): string {
	...
	return `[FOCUS]\nUse focuses to solve the user's request.\n\nFocus rules:\n- A focus is an operational mode that controls available tools and instructions.\n- Use the descriptions below to choose which focus a task needs.\n- Before substantive work, choose how to run that focus: enter_focus to do the work yourself in the current context, or agent to delegate it to a child agent that runs in the focus and returns only its final message.\n- Prefer agent when you only need the result, want to keep this context clean, or want to run work in parallel; pass the focus as the agent focus parameter instead of entering it yourself. Prefer enter_focus when you want to work step by step in that focus.\n- Auto focuses may be entered without asking the user.\n- Do not ask the user for information that can be discovered after entering an appropriate focus.\n- If a needed capability is not visible, first check whether another available focus exposes it.\n\nAvailable focuses:\n${formatFocusList(focuses)}`;
}
...
function buildFocusReminderPayloadWithTools(
	...
	const headline = activePrompt
		...
		: [
				...
				"If focus-scoped tools are needed, either call enter_focus to do the work yourself, or call agent with the target focus to delegate it without entering (keeping this context clean).",
			].join("\n");
	return {
		content: [
			headline,
