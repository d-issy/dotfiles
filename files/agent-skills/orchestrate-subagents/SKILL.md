...
- Choose from [Presets and layout](references/presets-and-layout.md). Unless the user specifies an effort level, use the lowest available effort for the selected model family. Use `pi` for Pi testing (no model override) and `sol` for web research.
...
```sh
herdr-subagents start architect=opus researcher=sol
```
